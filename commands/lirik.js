...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!lyrics)
      return client.sendTime(
        message.channel,
        `**Lirik yang dicari gaada -** \`${SongTitle}\``
      );
    lyrics = lyrics.split("\n"); //spliting into lines
    ...
    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`Lirik buat: ${SongTitle}`, client.botconfig.IconURL)
  ...
  SlashCommand: {
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "Masukin judul lagunya ",
        required: false,
      },
    ],
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      let Pages = SplitedLyrics.map((ly) => {
        let em = new MessageEmbed()
          .setAuthor(`Lirik untuk: ${SongTitle}`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(ly.join("\n"));
